...
export const createPost = async (request, response) => {
	try {
		const { title, description, titleBg } = request.body;
		let image;
		...
		const newPost = new Post({ title, description, image, titleBg });
		await newPost.save();
...
export const updatePost = async (request, response) => {
	try {
		const updatedPost = await Post.findByIdAndUpdate(request.params.id, request.body, {
			new: true,
		});
